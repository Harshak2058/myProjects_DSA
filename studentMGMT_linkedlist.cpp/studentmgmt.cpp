...
struct Student {
    ...
    int rollNumber;
    string course;
    double percentage; // Changed from grades to percentage
    Student* next;
}* head = NULL,
*temp = NULL,
* loc = NULL;
...
void addStudent(string name, int rollNumber, string course, double percentage) {
    // Create a new student node
    Student* newStudent = new Student;
    newStudent->name = name;
    newStudent->rollNumber = rollNumber;
    newStudent->course = course;
    newStudent->percentage = percentage; // Store percentage instead of grades
...
void searchStudent(int rollNumber) {
    ...
    while (temp != NULL) {
        if (temp->rollNumber == rollNumber) {
            ...
            cout << "Percentage: " << temp->percentage << "%" << endl; // Display as percentage
            found = true;
            break;
        }
        temp = temp->next;
...
void displayStudents() {
    ...
    // Display header
    cout << setw(15) << "Name" << setw(15) << "Roll Number" << setw(15) << "Course" << setw(15) << "Percentage" << endl;
    cout << "--------------------------------------------------------------" << endl;

    // Traverse and display each student's details
    temp = head;
    while (temp != NULL) {
        cout << setw(15) << temp->name << setw(15) << temp->rollNumber << setw(15) << temp->course << setw(15) << temp->percentage << "%" << endl;
        temp = temp->next;
    }
}
    ...
    do {
        ...
        switch (ch) {
            case 1:
                ...
                cin >> course;
                cout << "Percentage: ";
                cin >> percentage; // Input percentage instead of grades
                addStudent(name, rollNumber, course, percentage);
                break;
